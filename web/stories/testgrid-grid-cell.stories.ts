...
const Template: Story<Args> = ({ icon = '', status = '' }: Args) =>
...
PassWithErrors.args = {
  icon: 'E',
  status: TestStatus[TestStatus.PASS_WITH_ERRORS],
};
export const PassWithSkips = Template.bind({});
PassWithSkips.args = {
  icon: 'S',
  status: TestStatus[TestStatus.PASS_WITH_SKIPS],
};
export const Running = Template.bind({});
Running.args = { icon: 'R', status: TestStatus[TestStatus.RUNNING] };
export const CategorizedAbort = Template.bind({});
CategorizedAbort.args = {
  icon: 'A',
  status: TestStatus[TestStatus.CATEGORIZED_ABORT],
};
...
CategorizedFail.args = {
  icon: 'F',
  status: TestStatus[TestStatus.CATEGORIZED_FAIL],
};
